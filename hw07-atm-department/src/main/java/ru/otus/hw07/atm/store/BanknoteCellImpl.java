...
public class BanknoteCellImpl implements BanknoteCell {
    ...
    public void add(Banknote banknote) {
        if (this.banknote.faceValue() != banknote.faceValue())
            throw new IllegalArgumentException("Not supported face value");
        banknotes.add(banknote);
    }

    @Override
    public Collection<Banknote> extract(int count) {
        if (count <= 0 || count > banknotes.size())
            throw new IllegalArgumentException("Incorrect count of banknotes");
    ...
    public int getBalance() {
        return this.count() * this.faceValue();
    }
}
